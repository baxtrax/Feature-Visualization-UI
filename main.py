...
import numpy as np
import pandas as pd
from helpers.models import LayerTypes, ModelTypes, get_all_instances, get_feature_map_sizes, get_layer_by_name, setup_model
from helpers.manipulation import DatasetNormalizations, create_random_image, tensor_to_numpy_img
import gradio as gr
...
model, model_layers = None, None
layer_feature_map_sizes = None
selected_layer = None
image_block = None

def generate(lr, epochs, image_num, channel, nodeX, nodeY, node, progress=gr.Progress(track_tqdm=True)):

    param_f = lambda: param.image(image_num) # Image setup
    ...
    match type(selected_layer[1]):
        case nn.Conv2d:
            if (channel is not None and # Node Specific
                ...
                image = np.array(render.render_vis(model, 
                                                   objectives.channel(selected_layer[0], channel), 
                                                   thresholds=(epochs, ),
                                                   show_image=False,
                                                   optimizer=optimizer,
                                                   param_f=param_f)).squeeze(0).squeeze(0)
                ...
                image = np.array(render.render_vis(model, 
                                                   lambda m: torch.mean(torch.pow(-m(selected_layer[0]).cuda(), torch.tensor(2).cuda())).cuda(), 
                                                   thresholds=(epochs, ),
                                                   show_image=False,
                                                   optimizer=optimizer,
                                                   param_f=param_f)).squeeze(0).squeeze(0)
        ...
        case nn.Linear:
            if (node is not None): # Node Specific
                image = np.array(render.render_vis(model, 
                                                   objectives.channel(selected_layer[0], node), 
                                                   thresholds=(epochs, ),
                                                   show_image=False,
                                                   optimizer=optimizer,
                                                   param_f=param_f)).squeeze(0).squeeze(0)
...
with gr.Blocks(theme=gr.themes.Soft(primary_hue=custom_color, 
    ...
    with gr.Row(): # Lower inputs and outputs
        with gr.Column(): # Inputs
            ...
            model_dd = gr.Dropdown(label="Model", 
                                   ...
                                   choices=[m.name for m in ModelTypes])
    ...
    def on_model(evt: gr.SelectData, progress=gr.Progress()):
        ...
        global model
        model = setup_model(ModelTypes[evt.value])
        progress(0.25, desc="Getting layers names and details...")

        # Layer agnostic logic
        global model_layers
        model_layers = list(get_model_layers(model, getLayerRepr=True).items())
        #TODO Use F strings
        ...
        for i in range(len(model_layers)):
            layer = get_layer_by_name(model, model_layers[i][0])
        ...
        layer_feature_map_sizes = get_feature_map_sizes(create_random_image((227, 227),
                               DatasetNormalizations.CIFAR10_MEAN.value,
                               DatasetNormalizations.CIFAR10_STD.value).clone(), 
                                                model, 
        ...
        match type(selected_layer[1]):
            case nn.Conv2d:
                ...
                return {layer_text:  gr.update(visible=True),
                        channel_num: gr.update(info=f"""Values between {channel_num.minimum}-{channel_num.maximum}""", visible=True,  value=None),
                        nodeX_num:   gr.update(info=f"""Values between {nodeX_num.minimum}-{nodeX_num.maximum}""", visible=True,  value=None),
                        nodeY_num:   gr.update(info=f"""Values between {nodeY_num.minimum}-{nodeY_num.maximum}""", visible=True,  value=None),
            ...
            case nn.Linear:
                ...
                return {layer_text:  gr.update(visible=True),
                        ...
                        node_num:    gr.update(info=f"""Values between {node_num.minimum}-{node_num.maximum}""", visible=True, value=None)}
            case _:
    ...
    model_dd.select(lambda t: gr.Radio.update(visible=True), outputs=layer_dd)
    model_dd.select(on_model, outputs=layer_dd)

    # TODO: Make button invisible always until layer selection
    layer_dd.select(lambda t: gr.Button.update(visible=True), outputs=confirm_btn)
    layer_dd.select(on_layer, outputs=[layer_text,
                                       channel_num, 
                                       nodeX_num, 
                                       nodeY_num, 
                                       node_num])

    confirm_btn.click(fn=generate, inputs=[lr_sl, 
                                           epoch_num,
                                           image_num,
                                           channel_num, 
                                           nodeX_num, 
